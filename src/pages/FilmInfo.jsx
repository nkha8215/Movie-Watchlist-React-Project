...
const FilmInfo = () => {
    ...
    const removeWatchlist = () => {
        dispatch(
            removeFromWatchlist(film.imdbID)
        )
    }

    useEffect(() => {
        async function fetchFilmById() {
            const { data } = await axios.get(`https://www.omdbapi.com/?i=${id}&apikey=33132354`)
            console.log(data)
            setFilm(data)
        }
        fetchFilmById()
    ...
    return (
        <>
            <section className="film__infoPage">
                <div className="cover__overlay">
                    <div className="cover__overlay--top">
                        <figure className="cover__image--wrapper">
                            <img src={film.Poster} alt="" className="cover__image" />
                        </figure>
                        <div className="filmNameGenre__container">
                        <h1 className="filmName">{film.Title}</h1>
                            <div className="genres">
                                {film.Genre && film.Genre.split(',').map((genre, index) => (
                                    <Link 
                                        key={index}
                                        to={`/browse/${id}`} 
